#!/usr/bin/env python3
"""ws_proxy_multi.py — Multi-asset tick capture with per-asset browser profiles."""

import asyncio, json, logging, argparse, re, time
...
class TradeExecutor:
    ...
    async def execute(self, data):
        ...
        try:
            ...
            if btn:
                await btn.click()
                logger.info(f"Trade executed: {direction} R${amt}")
...
async def handle_trade(req):
    ...
    try:
        data = await req.json()
        res = await _trade_exec.execute(data)
...
async def start_http():
    ...
    site = web.TCPSite(runner, 'localhost', TRADE_PORT)
    await site.start()
    logger.info(f"HTTP trade server on port {TRADE_PORT}")
    ...
    while True:
        ...
        try:
            ...
            if sel:
                ...
                m = re.search(r'(\d+)%\s*[\n\s]+(\d+)%', txt)
                if m:
                    buy_pct = int(m.group(1)); sell_pct = int(m.group(2))
                    majority = "BUY" if buy_pct > sell_pct else "SELL"
                    async with httpx.AsyncClient() as cl:
                        await cl.post('http://localhost:8000/api/ingest/sentiment',
                                       json={'majority': majority, 'majority_percent': max(buy_pct, sell_pct),
                                             'asset': ASSET}, timeout=5)
        except: pass
...
async def run_browser():
    if not COOKIES_PATH.exists():
        logger.error(f"Cookies missing: {COOKIES_PATH}")
        return
    raw = json.loads(COOKIES_PATH.read_text())
    cookies = [{'name': c['name'], 'value': c['value'],
                'domain': c.get('domain', '.qxbroker.com'), 'path': c.get('path', '/'),
                'expires': c.get('expires') if isinstance(c.get('expires'), (int, float)) and c.get('expires', 0) > 0 else None,
                'httpOnly': c.get('httpOnly', False), 'secure': c.get('secure', False),
                'sameSite': c.get('sameSite', 'Lax')} for c in raw]
    logger.info(f"Loaded {len(cookies)} cookies, profile={PROFILE}")
    stealth = Stealth()
    while True:
        try:
            async with async_playwright() as p:
                ...
                global _trade_exec
                _trade_exec = TradeExecutor(page)
                asyncio.create_task(start_http())
                await page.goto(TARGET_URL, wait_until='domcontentloaded', timeout=60000)
                logger.info("Page loaded, waiting for ticks...")
